...
def register():
    if request.method=='POST':
        ...
        try:
            ...
            if count_email[0]==0:
                generated_otp=genotp() # S1bA2r
                userdata={'username':username,'useremail':useremail,'userpassword':userpassword,'user_phone':usercontact,'generated_otp':generated_otp}
...
def viewallnotes():
    ...
    try:
        ...
        if user_id:
            ...
            allnotesdata=cursor.fetchall() # [(1,'The Last Leaf','2026-05-21'),(2,.....)]
            print(allnotesdata)
            cursor.close()
        else:
            flash('Could not verify user')
...
def viewnotes(nid):
    if not session.get('user'):
        flash('To access Dashboard Features Please login')
        return redirect(url_for('login'))
    try:
        cursor=mydb.cursor(buffered=True)
        cursor.execute('Select userid from userdata where useremail=%s',[session.get('user')])
        ...
        if user_id:
            ...
            storednotesdata=cursor.fetchone() # [(1,'The Last Leaf','2026-05-21'),(2,.....)]
            cursor.close()
        else:
            flash('Could not verify user')
...
def viewallfiles():
    ...
    try:
        ...
        user_id=cursor.fetchone() # (1,) or (2,)
        if user_id:
            cursor.execute('select filesid,filename,created_at from filesdata where userid=%s',[user_id[0]])
            allfilesdata=cursor.fetchall() # [(1,'The Last Leaf','2026-05-21'),(2,.....)]
            cursor.close()
        else:
            flash('Could not verify user')
            return redirect(url_for('dashboard'))
...
def viewfile(fid):
    ...
    try:
        ...
        user_id=cursor.fetchone() # (1,) or (2,)
        if user_id:
            cursor.execute('select filesid,filename,filedata,created_at from filesdata where userid=%s and filesid=%s',[user_id[0],fid])
            storedfiledata=cursor.fetchone() # [(1,'The Last Leaf','2026-05-21'),(2,.....)]
            cursor.close()
        else:
            flash('Could not verify user')
            return redirect(url_for('dashboard'))
...
def deletefiles(fid):
    ...
    try:
        ...
        user_id=cursor.fetchone() # (1,) or (2,)
        if user_id:
            cursor.execute('delete from filesdata where userid=%s and filesid=%s',[user_id[0],fid])
            mydb.commit()
            cursor.close()
        else:
            flash('Could not verify user')
